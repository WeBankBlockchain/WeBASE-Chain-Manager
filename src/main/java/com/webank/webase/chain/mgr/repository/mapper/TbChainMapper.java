...
public interface TbChainMapper {
    ...
    @Insert({ "<script>", "insert into tb_chain (chain_id, ", "chain_type, description, ", "create_time, modify_time, ", "version, consensus_type, ", "storage_type, chain_status, ", "webase_sign_addr)", "values<foreach collection=\"list\" item=\"detail\" index=\"index\" separator=\",\">(#{detail.chainId,jdbcType=INTEGER}, ", "#{detail.chainType,jdbcType=TINYINT}, #{detail.description,jdbcType=VARCHAR}, ", "#{detail.createTime,jdbcType=TIMESTAMP}, #{detail.modifyTime,jdbcType=TIMESTAMP}, ", "#{detail.version,jdbcType=VARCHAR}, #{detail.consensusType,jdbcType=VARCHAR}, ", "#{detail.storageType,jdbcType=VARCHAR}, #{detail.chainStatus,jdbcType=TINYINT}, ", "#{detail.webaseSignAddr,jdbcType=VARCHAR})</foreach></script>" })
