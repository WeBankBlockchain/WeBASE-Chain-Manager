...
<template>
    <div>
        <v-content-head :headTitle="'合约列表'"></v-content-head>
        <div class="module-wrapper">
            ...
            <div class="search-table">
                <el-table :data="contractList" tooltip-effect="dark" v-loading="loading"  stripe>
                      <el-table-column prop="contractId" label="合约ID" show-overflow-tooltip width="100" >
                        <template slot-scope="scope">
                            <!-- <i class="wbs-icon-copy font-12 copy-public-key" @click="copyPubilcKey(scope.row.contractId)" title="复制合约地址"></i> -->
                            <span>{{scope.row.contractId}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="contractAddress" label="合约地址" show-overflow-tooltip width="150" align="center">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="contractPath"  label="合约目录" show-overflow-tooltip width="135" align="center">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="contractStatus" label="合约状态" show-overflow-tooltip width="135" align="center">
                        <template slot-scope="scope">
                            <span>{{isDeploy(scope.row.contractStatus) }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="contractAbi" label="合约ABI" show-overflow-tooltip width="400" align="center">
                        <template slot-scope="scope">
                            <!-- <i class="wbs-icon-copy font-12 copy-public-key" v-if='scope.row.contractAbi' @click="copyPubilcKey(scope.row.contractAbi)" title="复制合约ABI" ></i> -->
                            <span>{{scope.row.contractAbi}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="contractBin" label="合约BIN" show-overflow-tooltip width="400" align="center">
                        <template slot-scope="scope">
                            <!-- <i class="wbs-icon-copy font-12 copy-public-key" v-if='scope.row.contractBin' @click="copyPubilcKey(scope.row.contractBin)" title="复制合约BIN" ></i> -->
...
import { queryContractListUsingPOST } from "@/api/api"
...
export default {
    ...
    methods: {
        isDeploy(num){
        if(num==1){return '未部署'}else{return "已部署"}
        },
        ...
        getList(){
            ...
            queryContractListUsingPOST(data).then(
                res=>{
                if (res.data.code == 0) {
                    ...
                    this.contractList.forEach(function(item,index,arr){
                      arr[index].createTime= format(item.createTime,"yyyy-MM-dd HH:mm:ss")
                    })                  
                } else {
                   this.$message({
                        type: "error",
