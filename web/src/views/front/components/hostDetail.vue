        ...
        <div class="module-wrapper">
            <div class="more-search-table">
                <div class="search-item">
                    <span>显示日期</span>
                    <el-date-picker v-model="currentDate" type="date" placeholder="选择日期" :picker-options="pickerOption" 
                    format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd" :default-value="`${Date()}`" class=" select-32" @change="changeCurrentDate">
                    </el-date-picker>
                </div>
                <div class="search-item">
                    <span>对比日期</span>
                    <el-date-picker v-model="contrastDate" type="date" placeholder="选择日期" :picker-options="pickerOption" 
                    format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd" class=" select-32" @change="changeContrastDate">
                    </el-date-picker>
                </div>
                <div class="search-item">
                    <span>起止时间</span>
                    <el-time-picker is-range v-model="startEndTime" range-separator="至"
                     start-placeholder="开始日期" end-placeholder="结束日期" placeholder="选择时间范围" class="time-select-32">
                    </el-time-picker>
                </div>
                <div class="search-item">
                    <span>数据粒度</span>
...
import { getRatio, getMointorInfo } from "@/api/api";
...
export default {
    ...
    methods: {
        ...
        getChartData() {
            ...
            this.sureing = true;
            var reqData = {
                    front: this.nodesQuery.frontId
                },
                reqQurey = {};
            reqQurey = this.chartParam;
            getRatio(this.nodesQuery.frontId, reqQurey)
                .then(res => {
        ...
        getHealthData() {
            ...
            this.sureing = true;
            var reqData = {
                    frontId: this.nodesQuery.frontId
                },
                reqQurey = {};
            reqQurey = this.chartParam;
            getMointorInfo(this.nodesQuery.frontId, reqQurey)
                .then(res => {
                    this.loading = false;
                    this.sureing = false;
                    if (res.data.code === 0) {
        ...
        timeParam() {
            ...
            this.chartParam.groupId = this.$route.query.groupId;
        }
    }
};
</script>
